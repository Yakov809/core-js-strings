...
function removeFirstOccurrences(str, value) {
  const firstSlice = str.indexOf(value);
  const secondSlice = firstSlice + value.length;
  return str.includes(value)
    ? `${str.slice(0, firstSlice)}${str.slice(secondSlice)}`
    : str;
}
...
function removeLastOccurrences(str, value) {
  const firstSlice = str.lastIndexOf(value);
  const secondSlice = firstSlice + value.length;
  return str.includes(value)
    ? `${str.slice(0, firstSlice)}${str.slice(secondSlice)}`
    : str;
...
function isPalindrome(str) {
  return (
    ...
    [...str.replace(/[!?, ]/g, '').toLowerCase()].reverse().join('')
  );
}
...
function invertCase(str) {
  ...
  [...str].forEach((elem) => {
    if (elem.toLowerCase() === elem) {
      resStr.push(elem.toUpperCase());
    } else {
      resStr.push(elem.toLowerCase());
    }
  });
  return resStr.join('');
}
